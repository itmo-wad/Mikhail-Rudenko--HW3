...
app=Flask(__name__)
app.config['SECRET_KEY'] = "iluvtrap"
        ...
        if user != None:
            if username == user['username']:
                if password == user['password']:
                    ...
                    flash('Some data is invalid', 'error')  
            else:
                flash('Some data is invalid', 'error')
        else:
            flash('Some data is invalid', 'error')
...
def signout():
    ...
    flash('You successfully Sign Out!', 'message')
    return response
...
def input():
    if request.method == 'POST':
        username = request.cookies.get('username', '')
        note.insert_one({'username':username , 'note':request.form['post']})
